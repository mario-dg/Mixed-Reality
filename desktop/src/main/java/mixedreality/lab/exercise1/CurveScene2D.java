...
public class CurveScene2D extends Scene2D {
    ...
    public JPanel getUserInterface() {
        ...
        panel.add(new JLabel("Number control points:"));
        JSlider sliderNumControlPoints = new JSlider(3, 10, 4);
        sliderNumControlPoints.addChangeListener(e -> {
                    numberControlPoints = sliderNumControlPoints.getValue();
                    repaint();
                }
        );
        panel.add(sliderNumControlPoints);

        // Randomize control points
        JButton buttonRandomizeControlPoints = new JButton("Randomize control points");
        buttonRandomizeControlPoints.addActionListener(e -> {
            this.curve.setControlPoints(GetRandomControlPoints(numberControlPoints));
            repaint();
        });
        panel.add(buttonRandomizeControlPoints);
        return panel;
    }
    ...
    private Vector2f[] GetRandomControlPoints(int numControlPoints) {
        var controlPoints = new Vector2f[numControlPoints];
        for (var i = 0; i < numControlPoints; i++) {
            controlPoints[i] = new Vector2f(RandomFloat(), RandomFloat());
        }
